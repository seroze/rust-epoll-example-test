...
impl RequestContext {
    ...
    fn read_cb(&mut self, key: u64, epoll_fd: RawFd) -> io::Result<()> {
        ...
        match self.stream.read(&mut buf) {
            Ok(_) => {
                // we are reading the initial packet from stream and setting our content length
        ...
        // we are appending whatever data we got from the stream
        self.buf.extend_from_slice(&buf);

        // Ideally this should match the content length know 
    ...
    fn parse_and_set_content_length(&mut self, data: &str) {
        ...
        // we are only handling HTTP requests from the tcpStream 
...
/**
 * Utility methods to interact with epoll syscall
 */
...
/***
 * EPOLLONESHOT => event of interest gets deleted after os has notified
 * EPOLLIN => events of read interest only
 * EPOLLOUT => events of write interest only 
 */
